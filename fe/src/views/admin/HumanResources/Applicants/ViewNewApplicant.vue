	...
	<div class="container mb-5">
			...
			<div class="card p-5">
				...
				<div class="row">
					...
					<div class="col-6">
						...
						<h3 class="">{{ data.first_name }} {{ data.middle_name }} {{ data.last_name }}</h3>
						<p class="fw-light mt-3">
		...
		<b-modal id="approveModal" centered title="Approved - Schedule an interview">
			...
			<template #modal-footer="{ cancel }">
				...
				<b-button variant="success" v-on:click.prevent="" :disabled="isLoading" @click="approveApplicant">
					Confirm
				</b-button>
			</template>
		</b-modal>
		...
		<b-modal id="rejectModal" centered title="Reject Applicant">
			...
			<template #modal-footer="{ cancel }">
				...
				<b-button variant="danger" v-on:click.prevent="" :disabled="isLoading">
					Confirm
				</b-button>
			</template>
		</b-modal>
...
<script>
	...
	export default {
		...
		created() {
			document.title = 'New Applicant - ' + this.view_new_applicant.data.user.info.first_name + ' ' + this.view_new_applicant.data.user.info.last_name;
		},
		async mounted() {
			this.isLoading = true;
			...
			this.data = {
				...
				job_description: this.view_new_applicant.data.jobapplied.job_description,
				job_skills: this.view_new_applicant.data.jobapplied.job_skills,
				job_type: this.view_new_applicant.data.jobapplied.job_type,
				job_work_location: this.view_new_applicant.data.jobapplied.job_work_location,
				job_min_experience: this.view_new_applicant.data.jobapplied.job_min_experience,
				job_min_qualification: this.view_new_applicant.data.jobapplied.job_min_qualification,
				...
				seminars_trainings: this.view_new_applicant.data.user.info.seminars_trainings,
				work_experiences: this.view_new_applicant.data.user.info.work_experiences,
			};
